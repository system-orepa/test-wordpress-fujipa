		...
		<tr class="booking-form-people-number-row">
			...
			<td><?php
				...
				<?php if (isset($this->errmsg['count'])) : ?><div class="error-message"><?php echo $this->errmsg['count'] ?></div><?php endif; ?>
                 ...
                 <!-  注意事項追加 added by kazuya.okamoto 2019.11.04 ->

                <div class="notice-comment-box">
                <ul>
                    <li>WEBからのご予約は最大24名様までとなります。25名様以上のご予約につきましてはお電話にて承ります。</li>
                    <li>ご予約の際、大人の方は必ず1名以上必須となります。お子様のみでご予約いただくことはできません。</li>
                </ul>
                </div>
...
<?php // オプションの入力フォーム出力
	...
	<div id="action-button" style="text-align: center">
        ...
        <div class="action-button" style="text-align: center">
              <input class="btn-top-radius" type="button" value="前に戻る" onclick="history.back()">
        </div>
...
?>
	...
	<?php foreach ($column_order as $column) : $column_use = $this->reserve['column'][$column]; if (0 < $column_use) :
		...
		case 'name' : ?><tr>
                ...
                <div class="notice-comment-box">
                <ul>
                    <li>お名前は性・名ともに7文字以内でご入力をお願いいたします。</li>
                </ul>
                </div>
		...
		case 'tel' : ?><tr>
			...
			<td>
                ...
                <!--  注意事項追加 added by kazuya.okamoto 2019.11.02 -->
                <div class="notice-comment-box">
                <ul>
                    <li>電話番号はハイフン（-）なしで、13桁以内でご入力をお願いいたします。</li>
                </ul>
                </div>
